...
export const dispatcher = (state, action) => {
  switch (action.type) {
    case GAME_STATE_ACTION_TYPE.CHECK: {
      // Validate payload
      // Схема { target: String, value: String }
      if (!action.payload) throw new Error("payload is required");
      if (Object.keys(defaultValue.board).indexOf(action.payload.target) === -1)
        throw new Error("payload.target has unexpected value");
      if (["x", "o"].indexOf(action.payload.value) === -1)
        throw new Error("payload.value is invalid. It expects `x` or `o`");
      ...
      if (action.payload.value === "o")
        newState.state = GAME_STATE.OPPONENTS_TURN;
    ...
    case GAME_STATE_ACTION_TYPE.CHANGE_DIFFICULTY: {
      // Validate Payload
      const validPayloads = Object.keys(GAME_DIFFICULTIES).map(
        k => GAME_DIFFICULTIES[k]
      );
